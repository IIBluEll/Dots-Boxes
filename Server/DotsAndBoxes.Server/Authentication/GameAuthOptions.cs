...
{
    public const string SECTION_NAME = "TestLogin";

    public bool Enabled { get; set; }

    public Guid UserId { get; set; }

    public string DisplayName { get; set; } = "PC 테스트";
}
    ...
    public TokenValidationParameters CreateValidationParameters()
    {
            ...
            LifetimeMinutes > 1440 )
        {
            throw new InvalidOperationException(
                "GameJwt configuration is missing or invalid. " +
                "Inject a random SigningKey of at least 32 bytes via server secrets.");
        }
...
public sealed record GameLoginResponse(
    Guid UserId ,
    string DisplayName ,
    string AccessToken ,
    DateTimeOffset ExpiresAtUtc);

public sealed record VerifiedGooglePlayer(
    string PlayerId ,
    string? DisplayName);
...
public sealed class GoogleAuthenticationException : Exception
{
    public GoogleAuthenticationException()
        : base("Google authentication was rejected.")
    {
    }
}
...
public sealed class GoogleUnavailableException : Exception
{
    public GoogleUnavailableException()
        : base("Google authentication is temporarily unavailable.")
    {
    }
}